...
void main(void)
{
    ...
    Lectura(INV, TAM);
    printf("\nIngrese operaciones a realizar. \n\t\t1 - Ventas \n\t\t 2 - Reabastecimiento \n\t\t 3 - Nuevos Productos \n\t\t 4 - Inventario \n\t\t 0 - Salir: ");
    scanf("%d", &OPE);
    while (OPE)
    {
        switch (OPE)
        ...
        case 3:
            ...
            /* Se pasa el parámetro por referencia, porque se puede modificar elnúmero
...
/* Esta funcion se utiliza para leer un arreglo unidimensional de tipo estrucututa
producto de T elementos. */
{
    int I;
    for (I=0; I<T; I++)
    {
...
obtener el total de las ventas, se actualiza el inventario. */
{
    ...
    {
        printf("\nNo existe un inventario la cantidad solicitada. Solo hay %d", A[I].existencia);
        printf("\nLos lleva 1 - Si 0 - no?: ");
        scanf("%d", &RES);
        if (RES)
        {
...
/* Esta función se utiliza para reabastecer al inventario. */
{
    ...
    while (CLA)
    {
        ...
        printf("nIngrese otra clave del producto -0 para salir-: ");
        scanf("%d", &CLA);
    }
}
...
que continúen ordenados. */
{
    ...
    while ((*T < 30) && (CLA))
    {
        ...
        else if (A[I].calve == CLA)
            printf("\nEl producto ya se encuentra en el inventario");
        else
        {
            for (J=*T; J>I; J--)
                /* Se inserta el nuevo producto en el arreglo. Se mueven una posición
                a la derecha los elementos del arreglo que tengan una clave de producto
                mayor a la ingresada. */
                A[J] = A[J-1];
            A[I].clave = CLA;
            printf("\tNombre:");
            fflush(stdin);
