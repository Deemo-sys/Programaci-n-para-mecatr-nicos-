...
void main(void)
{
    ...
    if ((ar = fopen("arc.txt", "r")) != NULL)  /*Se abre el archivo para lectura. */
        /* Observa que las dos instrucciones del programa 9.1 necesarias para abrir un archivo
        y verifica que este en realidad se haya abierto, se pueden agruar en una sola instruccion. */
    {
        while (!feof(ar))
