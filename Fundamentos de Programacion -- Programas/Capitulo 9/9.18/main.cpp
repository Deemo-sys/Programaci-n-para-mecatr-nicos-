...
/* Esta funcion mezcla, respetando el orden, dor archivos que se encuentran
ordenados en funcion de la matricula. */
{
    ...
    while (((!feof(ar)) || !b) && ((!feof(ar1)) || !b1))
    {
        ...
        if (b1) /* Si la bandera b1 esta encendida, se lee del archivo ar
                la matricula y las tres calificaciones del alumno. */
                {
                    fscanf(ar1, "%d", &mat1);
                    for (i=0; i<3; i++)
                        fscanf(ar1, "%f", &ca1[i]);
