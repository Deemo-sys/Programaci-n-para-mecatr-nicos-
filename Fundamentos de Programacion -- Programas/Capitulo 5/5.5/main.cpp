...
alumnos, obtiene la frecuencia de cada de las calificaciones y ademas
escribe cual es la frecuencia mas alta. */

const int TAM = 50;

void Lectura(int *, int);
void Frecuencia(int A[], int P, int B[], int T);  /* Prototipo corregido */
...
void main(void)
{
    ...
    Frecuencia(CAL, TAM, FRE, 6); /* Corregida la llamada a la funci�n */
    printf("\nFrecuencia de Calificaciones\n");
    Impresion(FRE, 6);
    Mayor(FRE, 6);
}
...
/* Esta funcion calcula la frecuencia de calificaciones. */
{
    ...
    for (I=0; I<P; I++)
        if ((A[I] >= 0) && (A[I] < 6))  /* Se valida que la calificacion sea correcta. */
            B[A[I]]++;     /* Incrementa las frecuencias */
}
