...
void main(void)
{
    ...
    scanf("%d", &ELE);
     RES = Buscar(VEC, TAM, ELE);  /* Se llama a la funcion que busca en el
    arreglo. */
    if (RES)
        /* Si RES tiene un valor verdadero -diferente de 0-, se escrbe la posicion
...
0. */
{
    ...
    if (BAN)
       RES = I + 1;
        /* Se asigna I+1 dado que las posiciones en el arreglo comienza desde
        cero. */
        else
        RES = BAN;
    return (RES);
