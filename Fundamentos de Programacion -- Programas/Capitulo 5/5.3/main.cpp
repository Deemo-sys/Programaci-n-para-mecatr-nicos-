...
void main(void)
{
    ...
    *IX = 15;   /* Z[2] se modifica, ahora vale 15. */
    printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z4 = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
    ...
    *IX += 1; /* Z[2] se vuelve a modificar, ahora vale 12. */
    ...
    IX = IX + 4; /* IX se modifica. Observa que el apuntador se mueve 4 posiciones y cae en una direcci�n afuera del arreglo. Esto ocasionar� un error. */ Y = *IX; /* Y se modifica, toma el valor (basura) de una celda incorrecta. Es un error que no se�ala el compilador del lenguaje C. */
    printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z4 = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
